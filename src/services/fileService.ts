 ...
 * @returns Extracted text content
 */
export async function processFile(filePath: string, fileType: string): Promise<string> {
  try {
    if (fileType === 'application/pdf') {
      return await extractTextFromPDF(filePath);
    } else if (fileType === 'text/markdown' || path.extname(filePath) === '.md') {
 ...
 */
export async function processFileAndAddToVectorStore(fileDoc: FileDocument): Promise<{
  filename: string;
  chunks: number;
